...
/* final */class Aegis128L implements KeyGenerator, DataEraser
{
    use HasRequirements;
    ...
    public function encrypt(
        ...
        }, $message, $nonce, constant('SODIUM_CRYPTO_AEAD_AEGIS128L_NPUBBYTES'));
    }
    ...
    public function decrypt(
        ...
        return $this->wrapDecryption(static function (string $ciphertext, string $nonce) use (&$key, $additionalData): string {
            return OptionalString::ofFalsable(sodium_crypto_aead_aegis128l_decrypt($ciphertext, $additionalData ?? '', $nonce, $key))->orElseThrow(
                static fn () => new Exception\CouldNotDecryptData('sodium_crypto_aead_aegis128l_decrypt', $ciphertext),
            );
        }, $ciphertext, $nonce, constant('SODIUM_CRYPTO_AEAD_AEGIS128L_NPUBBYTES'));
    }
}
